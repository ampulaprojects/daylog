...
ROLE_ASISTENT = "asistent"


def is_family(role) -> bool:
    """Člen rodiny (admin/user) — plný prístup k Liekom/Katalógu a smie meniť
    ktorýkoľvek záznam v zdieľanom rodinnom denníku. Asistent nie."""
    return role != ROLE_ASISTENT


def can_modify_entry(user, entry) -> bool:
    """Kto smie editovať/mazať daný záznam. Rodina (admin/user) hocijaký;
    asistent len vlastný (entry.user_id == user.id). Vlastníctvo sa teda
    vynucuje LEN pre asistenta — správanie admin/user sa nemení."""
