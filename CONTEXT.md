...
- PATCH sémantika pre /medications (diera A v plnom rozsahu): `update_medication()` prepisuje VŠETKY stĺpce hodnotami z `MedBody`, takže pole, ktoré UI nepošle, sa vynuluje na Pydantic default. Správne riešenie je načítať existujúci riadok na serveri a prepísať len skutočne poslané polia. DNES BEZ NALIEHAVOSTI — audit z Bloku 2A ukázal nulovú stratu dát (jediné neposielané pole bol `time_type` a ten bol NULL vo všetkých 24 riadkoch; odvtedy sa round-tripuje). Naliehavosť vznikne, keď pribudne pole bez widgetu
- Rovnaká trieda chyby cez druhý mechanizmus (widget nevie reprezentovať hodnotu z DB → uloženie ju prepíše): `days` vyriešené módom `neznamy`, `catalog_id` doplnením neaktívnej položky do dropdownu. Zostáva `kind` — `<select>` má len liek/vitamin/doplnok, neznámy druh by pri uložení spadol na „liek". Dnes bezpečné (v DB len liek a doplnok), ale je to tá istá mína
- Tenký endpoint pre výber z katalógu: `/catalog/list` vracia CELÉ položky vrátane `pil_info` (desiatky kB na položku). `meds.html` aj `index.html` si to klientsky orežú na 2–5 polí, takže funkčne to sedí, ale po sieti sa vlečú dáta, ktoré nikto nepotrebuje — aj na mobile. Správne by bol `/catalog/options` s `id` + `canonical_name`. Dnes bez naliehavosti
- Rybí olej (režim id17, „ráno, dáta 08:30-09:30" = časové ROZPÄTIE) a Probiotikum (id24, „po kaši" = pravidlo RELATÍVNE K UDALOSTI) — iné typy pravidiel než parita dňa, patria do samostatného bloku o časových pravidlách. Tam bude `time_type` prirodzený rozlišovač (presny/rozpatie/po_udalosti); stĺpec preto NEZAHADZOVAŤ
...
### 2026-08-06 — Blok 2B: preklad záznamu + nešpecifikovaný liek (NENASADENÉ)
- NÁLEZ, ktorý zmenil východisko: preklad UŽ EXISTOVAL. Model prekladal do `cleaned_text` sám od seba, hoci prompt o preklade nehovoril nič. A UI naň malo tlačidlo — „Navrhovaný text → Prijať" prepísalo `entries.text` prekladom a **ruský originál nenávratne zmazalo**. Bolo to striktne buď–alebo: buď originál, alebo preklad, nikdy oboje
- OPRAVA PRÍČINY, nie príznaku: `cleaned_text` robil dve rôzne práce naraz (oprava gramatiky pri SK vstupe, preklad pri RU vstupe). Prompt ich rozdelil — `cleaned_text` má teraz explicitné „NEPREKLADAJ" a zostáva v pôvodnom jazyku, preklad ide do nového poľa `text_sk`. Tým „Prijať" prestalo byť deštruktívne: znamená „prijmi opravu gramatiky v mojom jazyku"
- `entries` + 3 stĺpce: `text_sk`, `source_lang`, `translation_status`. `entries.text` je VŽDY pôvodný vstup a preklad sa ho nikdy nedotkne
- Tri stavy prekladu + NULL, zámerne explicitne: `nepotrebny` (bol po slovensky) / `hotovy` / `zlyhal` (cudzí jazyk rozpoznaný, preklad neprišiel) / NULL (starý záznam). `nepotrebny` a `zlyhal` majú OBIDVA `text_sk = NULL`, takže sa podľa neho rozlíšiť nedajú — na to je stavový stĺpec. Pri `zlyhal` sa loguje varovanie a UI to zobrazí, nie ticho
- UI: prepínač slovensky/originál nad textom záznamu, len keď je čo prepínať. Staré záznamy (status NULL) sa správajú presne ako predtým. Spätná migrácia sa NEROBÍ — vedomé rozhodnutie
- `med_terms.py` — NOVÝ modul, jediné miesto pravdy pre všeobecné výrazy + stavy prekladu. `GENERIC_MED_TERMS`, `normalize_term()` (casefold + NFD bez diakritiky), `is_generic()`, `normalize_status()`
- KRITÉRIUM je ZHODA SO ZOZNAMOM, nie absencia `catalog_id`: „liek", „vitamíny", „лекарство" áno; „Magm", „Mg", „Vit. C", „karнозín" NIE — tie majú `catalog_id` NULL kvôli preklepom a aliasom, čo je iný problém a iný blok. Porovnáva sa CELÝ normalizovaný reťazec, nie podreťazec (alias „Mg" by pároval aj v „600 mg Orfiril")
- „Ranné vitamíny podané až na obed" zámerne NEZHODNÉ — fakticky generické, ale ako celok v zozname nie je a podreťazcové párovanie sem nepustíme. Jediný taký riadok, historický
- Detekcia beží na SERVERI (`_enrich_with_catalog` → `ev.generic`), klient si zoznam výrazov nedrží — inak by prestal byť na jednom mieste
- Rozvinutie: „vitamíny" + výber {B-komplex, Vitamín C, Zinok} → 3 riadky `event_meds`, každý s vlastným `catalog_id`, ale **so ZACHOVANÝM `raw_name='vitamíny'`** na všetkých. Audit trail: navždy vidno, čo tam pôvodne stálo. `event_meds.specified_by_user = 1` (explicitný stĺpec, nie odvodenie — to by sa rozpadlo, keby niekto pridal „vitamíny" ako alias)
- PRESKOČENIE je plnohodnotný stav: kto nevie, čo to bolo, nemá hádať. Prázdny výber = riadok s `catalog_id` NULL presne ako predtým
- KONZERVATÍVNE PRAVIDLO: upresnenie sa použije LEN ak je v evente práve JEDEN generický riadok. Pri nule alebo viacerých sa ignoruje a **zaloguje sa varovanie** — radšej nedoplniť než doplniť k nesprávnemu riadku
- Editačná cesta je v rozsahu bloku, nie dlh: `update_entry_with_events()` maže a znova vytvára všetky `event_meds`, takže bez prenosu `specified_catalog_ids` by prvá editácia zmazala všetky doplnené lieky — presne mína z Bloku 2A. Test 30 to stráži. Zmena textu naopak `text_sk` VYNULUJE (radšej žiadny preklad než nesúhlasiaci)
- Testy: `tests/test_med_terms.py`, 33 testov. Celý balík 164 testov v 13 súboroch prechádza
- `compare_prompts.py` — NESPUSTENÝ skript na porovnanie starého a nového promptu na 5 reálnych textoch z produkcie (2 ruské od Iriny, 2 slovenské, 1 s reálnou opravou gramatiky). Unit testy overia kód, nie správanie modelu; toto je jediný spôsob, ako chytiť regresiu v extrakcii SLOVENSKÝCH textov. `OLD_PROMPT` je bajtovo zhodný s commitom c300e9c
- ZNÁMY FAKT (nediagnostikujeme spätne): časť ruských originálov mohla byť pred týmto blokom prepísaná prekladom cez „Prijať". Pôvodný vstup je dohľadateľný v `entries.llm_analysis` (celá surová odpoveď modelu vrátane `cleaned_text`) a dá sa vytiahnuť BEZ nového LLM volania
- Pri písaní promptu sa mi do neho omylom dostal cyrilický homoglyf `о` (U+043E) v slove „verne". Zachytené kontrolou pred commitom. V projekte, ktorý má homoglyfy ako známy problém v dátach, stojí za to kontrolovať aj vlastné zdrojáky

### 2026-08-05 — Blok 2A: kódovanie striedavého režimu (NENASADENÉ)
- PRÍČINA: `days` mala len 3 tvary, ktoré vedelo UI zostaviť (`kazdy_den`, `pri_krize`, CSV dní). Hodnotu `parny_datum` na riadku id23 `parseDaysToMode()` nezaradila, spadla na tichý fallback `kazdy_den` (starý meds.html:364), a `getDaysValue()` ju pri uložení odoslala. Server nemal na `days` žiadnu validáciu → prešlo čokoľvek. Stačilo otvoriť riadok a uložiť BEZ ZMENY
- DÔKAZ, nie dohad: CONTEXT.md s varovaním commitnutá 13:24:03Z, riadok id23 editovaný 13:35:05Z. `updated_at` mení len `update_medication()`, `set_medication_active()` a `reorder_medications()`; posledné dve `days` nesahajú → zmena musela ísť cez PUT /medications/{id}, teda cez /meds
- `med_rules.py` — NOVÝ modul, jediné miesto pravdy pre `days`. Konštanty, `DAYS_LABELS`, `normalize_days()`, `is_due()`, `format_days()`. Bez DB a bez LLM (vzor: med_parser.py). Fáza 2 bude importovať `is_due()`
- Serverová validácia: `@field_validator("days")` na `MedBody` → neznáma hodnota končí ako 422, nie tichý prepis. `None`/`''` sú CHYBA, nie default — chýbajúci vstup musí byť odlíšiteľný od platného „každý deň". Default v deklarácii ponechaný kvôli spätnej kompatibilite (klient, ktorý kľúč nepošle)
- `is_due('pri_krize')` → False (PRN nemá plánovaný deň, nikdy sa neohlási ako vynechaná). Neznáma hodnota → `InvalidDays`, NIE False — inak by Fáza 2 liek s pokazeným pravidlom ticho vynechala z porovnania
- PÁRNY DÁTUM = `day % 2`, deň v mesiaci, bez kotvy. Po 31. nasleduje 1. → dva nepárne dni po sebe (7× do roka) a jednodňová diera pre párne pravidlo. JE TO ZÁMER a testy 09/10 to komentujú: pravidlo, ktoré reálne platí, znie „pozri sa na dátum, je párny?" — softvér sa nesmie rozísť s človekom, ktorý drží dávkovač. Ozajstná alternácia ob deň by bola iná hodnota s kotviacim dátumom
- UI: 4. mód „Striedavo" s podvýberom Párny/Nepárny — rovnaký idióm ako „Konkrétne dni" + weekday-picker, nie `<select>` (5 tlačidiel v rade by sa na mobile zalomilo)
- NAJHODNOTNEJŠIA ČASŤ: mód `neznamy`. Hodnota z DB, ktorej UI nerozumie, sa ZOBRAZÍ so surovým textom a uloženie sa zablokuje, kým sa nezvolí platný mód. Zrušené sú OBA tiché fallbacky — aj ten pri „Konkrétne dni" bez označeného dňa. `getDaysValue()` vracia `null` namiesto výmyslu; volajúci musí zastaviť
- Zavreté dve odistené míny (audit MedBody × UI × DB, aktívna strata dát bola NULOVÁ): `time_type` sa neposielal ani z jednej save funkcie → `update_medication()` ho pri každom edite prepisoval na NULL. Teraz sa round-tripuje (bez widgetu a bez sémantiky — tá patrí do bloku o časových pravidlách). `catalog_id`: dropdown sa plní len z aktívnych položiek, takže riadok ukazujúci na deaktivovanú položku by po edite ticho stratil väzbu — teraz sa doplní a označí „— neaktívna"
- Testy: `tests/test_med_days.py`, 33 testov (logika, prechod mesiaca, odmietnutie neplatných hodnôt, round-trip regresia vrátane time_type a neaktívneho catalog_id, idempotencia migrácie). Celý balík 131 testov v 12 súboroch prechádza
- Migrácia `migrate_med_days.sql` NESPUSTENÁ. Aktívny je len id23 (D3K2 → parny_datum). **Zinok (id22) je zakomentovaný** — pre neho neexistuje doklad o pôvodnej hodnote (CONTEXT.md spomína iba id23) a dáta sú slabé, 3 zo 4 pozorovaní. Čaká na overenie u opatrovateľky
- ⚠ PORADIE NASADENIA: najprv kód, reštart, overiť „Striedavo" v prehliadači, AŽ POTOM SQL. Opačné poradie zopakuje pôvodnú chybu. Posledný krok — otvoriť id23 v /meds a uložiť bez zmeny — je jediný skutočný test pôvodnej chyby, lebo JS test runner v projekte neexistuje

### 2026-08-05
- INVENTÚRA LIEKOV pred Fázou 2 (štyri kroky, všetko dry-run → záloha → apply → nezávislé SQL → idempotencia): bez úplného zoznamu by Fáza 2 hlásila falošnú odchýlku pri každom doplnku mimo režimu a šum by pohltil skutočné signály
- Krok 1 — aliasy: "mozog" → Grasgevoerd Brein (10), "magnézium" → Tripla Magnesium (11), "karnosín" → Carnosine (9). Tri položky boli v katalógu a nikdy sa nepoužili, lebo sa diktuje slovenský opis namiesto názvu z krabičky. Časy to potvrdili: Magtein je večerný (20:00), "magnézium" chodí ráno a o 13:30 — iný produkt. Záloha pre-aliases2
- NEPRIDANÉ zámerne: alias "Mg" by pároval podreťazcom aj v "600 mg Orfiril" a ticho prepísal dávky Orfirilu na magnézium. Krátke aliasy sú pri podreťazcovom párovaní nebezpečné — pravidlo do budúcna
- Krok 2 — 13 nových položiek katalógu (11 → 24): 8 aktívnych doplnkov (Glycín, B-komplex, BX, Probiotikum, Rybí olej, Zinok, Vitamín C, Vitamín D3K2) + 5 historických neaktívnych (Srdčín, Losetan, Codony, Skenar, Oftalmilol — active=0, aby sa staré záznamy spárovali kvôli audit trailu, ale nezavádzali v aktívnych zoznamoch). Pred zápisom SIMULÁCIA match_catalog cez všetky raw_name: 0 mis-pairov, čiže žiadne existujúce správne párovanie sa nemení. Simulácia pred dátovým zápisom je vzor na opakovanie. Záloha pre-catalog-additions
- Krok 3 — parser: diagnostika ukázala, že problém NEEXISTUJE. Flag _generic_only je len diagnostická nálepka, ktorú žiadny volajúci nečíta; match_catalog páruje nezávisle od GENERIC. "vitamín C" bol nespárovaný len preto, že položka vtedy nebola v katalógu. Neopravovali sme nezlomené
- Krok 4 — re-pair: 72 riadkov event_meds dostalo catalog_id (45 nové položky + 27 alias-ciele). NULL 85 → 13, spolu 304 nezmenené, source nezmenené, 0 osirených. Železné pravidlo: UPDATE mal v podmienke catalog_id IS NULL, existujúce párovania sa nesmeli dotknúť; raw_name sa nemenil nikdy (audit trail). Záloha pre-repair. Zvyšných 13 NULL sú skutočné generiká ("vitamíny", "lieky", "podanie lieku") + schválené vynechania (Mg, Magm, "Vit. C" s bodkou)
- Krok 5 — režim: 13 nových riadkov med_schedule (11 → 24) podľa REÁLNEHO ZOZNAMU, ktorý Jan dáva asistentke — ten je odteraz zdroj pravdy, nie odhad z dát. Ranné okno 08:30 (mozog, Tripla Magnesium, B-komplex, rybí olej, BX), 13:30 (vit C, Tripla Magnesium 2. dávka, glycín, zinok, D3K2), 14:30 probiotikum, 20:00 (B6, Thiamin). Doplnené množstvo pri Carnosine 20:30 a dolinkované 2 krízové riadky (id5 → TISERCIN, id7 → Chlorprothixen). Záloha pre-schedule
- Zoznam pre asistentku zároveň vyriešil dátovú neistotu: výskyt ~50 % dní u B6/magnézia/mozgu NIE JE obdenný režim, ale nedôsledný zápis — všetko je denné. Presne to je rozdiel "nezapísané" vs "nepodané", ktorý má riešiť denný sumár
- PRINCÍP PRE FÁZU 2 (rozhodnuté): porovnávanie časov musí ísť cez TOLERANČNÉ OKNO rádovo ±90 min, nie na minútu. Časy sa reálne posúvajú podľa toho, kedy sa syn zobudí a ako ide deň; režimový čas je kotva, nie termín. Dávky sú od seba 4–5 hodín, takže široká tolerancia ich nikdy nezamení. Preto sme NEZOSÚLAĎOVALI existujúce režimové časy (08:00 vs 08:30 zo zoznamu) — je to irelevantný rozdiel
- ~~PASCA: med_schedule nevie alternujúce vzory. Vitamín D3K2 je uložený ako days="parny_datum" — hodnota, ktorú UI nepozná, riadok id23 sa NESMIE editovať cez /meds~~ — PASCA SA NAPLNILA 11 MINÚT PO TOM, ČO BOLA ZAPÍSANÁ (commit e209350 o 13:24:03Z, editácia riadku id23 o 13:35:05Z) a hodnota sa stratila. Rieši Blok 2A
- Stav po inventúre: katalóg 24 položiek, režim 24 riadkov (0 s catalog_id NULL), event_meds 291 z 304 riadkov spárovaných

### 2026-08-02
- Zoznam záznamov ukazoval len 50 najnovších (server-side default LIMIT 50, žiadny offset) — v DB je 152 záznamov od 27.6., staršie boli nedostupné. Doplnené stránkovanie: get_entries má offset, GET /entries limit (clamp 1..200) + offset, UI tlačidlo "Načítať staršie" pridáva ďalších 50 k existujúcim
- Do ORDER BY pribudlo e.id DESC ako posledné kritérium: pri zhodnom entry_date aj entry_time nebolo poradie deterministické a offset by záznam zopakoval alebo vynechal. Toto je jadro opravy, nie kozmetika
- Metadáta stránkovania idú v HTTP hlavičkách X-Total-Count a X-Has-More, telo odpovede zostalo holým poľom — spätná kompatibilita, kto hlavičky nečíta, nič nezbadá
- Filtre (dátum, typ eventu, triedenie) presunuté na server: predtým filtrovali len načítané záznamy, takže pod stránkovaním klamali — filter "1.7.–10.7." ukázal prienik s načítaným oknom, nie júl. Zdieľaná _entries_where() pre get_entries aj count_entries (rozídené WHERE by znamenalo klamúci total; test to výslovne kontroluje). Typ eventu cez EXISTS, nie JOIN — inak by sa záznam s viacerými zhodnými eventmi vrátil duplicitne
- Triedenie na serveri s id DESC chvostom v oboch variantoch (event/created); klientské triedenie pod stránkovaním preusporiadalo len načítané okno, čo nie sú globálne najnovšie
- Režim "Len eventy" zostal klientský, ale počítadlo je čestné: "X eventov z N načítaných záznamov (Y celkovo)". Stránkovanie eventov je iná jednotka strany — samostatná téma, ak sa ukáže potreba
- Neplatný dátum → 400 so SK hláškou (tichý ignore by ukázal iné dáta, než používateľ zadal); neznámy event_type sa ticho zahodí (uzavretý enum, znamená stale klienta); neznámy sort → default
- Zmazané tri osirené klientské helpery (sortedEntries, dateFilteredEntries, filteredEntries, 27 riadkov) — mŕtvy kód po presune je pasca, nie úspora
- tests/test_entries_paging.py (6) + tests/test_entries_filters.py (8). Spolu 98 testov
- OTVORENÉ: filtre nie sú v URL ani localStorage — po refreshi sa resetujú (vedomé rozhodnutie, rozsah bloku)
